...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    List<Category> findByUser(@Param("user") CustomUser customUser);
    ...
    @Query("SELECT u FROM Category u where u.name = :name and u.userName=:userName")
    Category findByName(@Param("name") String name, @Param("userName") String userName);

    @Query("SELECT COUNT(u) FROM Category u where u.userName=:userName")
    long countByCategories(@Param("userName") String userName);
}

...
public interface WordRepository extends JpaRepository<Word, Long> {
    ...
    @Query("SELECT u FROM Word u where u.name = :name and u.userName=:userName")
    Word findByName(@Param("name") String name, @Param("userName") String userName);

    @Query("SELECT COUNT(u) FROM Word u where u.categoryName=:categoryName and u.userName=:userName")
    long countByWords(@Param("categoryName") String categoryName, @Param("userName") String userName);
}
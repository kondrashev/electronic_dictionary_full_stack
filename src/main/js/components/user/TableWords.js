...
function TableWords(props) {
    ...
    React.useEffect(() => {
        let data = {
            url: `/get/words?page=${values.numberPageWords - 1}&categoryName=${values.currentNameCategory}&userName=${sessionStorage.userName}`,
            values: values,
            setValues: setValues
        }
        getWords(data);
    ...
    const deleteWords = () => {
        let data = {
            url: `/delete/words?categoryName=${values.categoryName}`,
            values: values,
            setValues: setValues,
